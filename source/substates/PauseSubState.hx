...
{
	var grpMenuShit:FlxTypedGroup<Alphabet>; //poopy

	var menuItems:Array<String> = [];
	var menuItemsOG:Array<String> = ['Resume', 'Restart Song', 'Do Nothing', 'Chart Editor', 'Change Difficulty', 'Options', 'Exit to menu'];
	var difficultyChoices = [];
	var curSelected:Int = 0;

	var nada:FlxSprite;
	var nadaTween:FlxTween = null;

	var pauseMusic:FlxSound;
	var practiceText:FlxText;
	var skipTimeText:FlxText;
	var skipTimeTracker:Alphabet; //poopy
	...
	override function create()
	{
		...
		nada = new FlxSprite().loadGraphic(Paths.image('nothing'));
		nada.antialiasing = ClientPrefs.data.antialiasing;
		nada.setGraphicSize(Std.int(nada.width * 2));
		nada.updateHitbox();
		nada.screenCenter();
		...
		var ballsTxt = inVid ? '$cutscene_branding Paused' : Language.getPhrase("blueballed", "{1} Bolitas Tristes", [PlayState.deathCounter]);
		...
		grpMenuShit = new FlxTypedGroup<Alphabet>(); //poopy
		add(grpMenuShit);
		...
		if (controls.ACCEPT && (cantUnpause <= 0 || !controls.controllerMode))
		{
			...
			switch (daSelected)
			{
				...
				case "Do Nothing":
					...
					add(nada);
					nada.alpha = 1;
					nadaTween = FlxTween.tween(nada, {alpha: 0}, 1, {ease: FlxEase.linear});
				case 'Chart Editor':
	...
	function regenMenu():Void {
		...
		{
			var obj:Alphabet = grpMenuShit.members[0]; //poopy
			obj.kill();
		...
		for (num => str in menuItems) {
			var item = new Alphabet(90, 320, Language.getPhrase('pause_$str', str), true); //poopy
